                        ...
                        <tbody>
                            @forelse($users as $user)
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="flex-shrink-0">
                                                <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&background=2d5a05&color=fff&size=40" 
                                    ...
                                    <td><i class="ti ti-mail me-1 text-muted"></i>{{ $user->email }}</td>
                                    <td>
                                       @if($user->permissions->count() > 0)
                                            @foreach($user->permissions as $permission)
                                                <span class="badge bg-light-info text-info me-1">
                                                    <i class="ti ti-shield me-1"></i>{{ $permission->name_ar ?? $permission->name }}
                                                </span>
                                            @endforeach
                                        @else
                                            <span class="text-muted"><i class="ti ti-shield-off me-1"></i>لا يوجد أدوار</span>
                                        @endif
                                    </td>
                                    <td>
                                        @if($user->clusters->count() > 0)
                                            ...
                                            @endforeach
                                        @else
                                            <span class="text-muted"><i class="ti ti-map-off me-1"></i>لا يوجد تجمعات</span>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <i class="ti ti-calendar me-1 text-muted"></i>{{ $user->created_at->format('Y-m-d') }}
                                        <br><small class="text-muted">{{ $user->created_at->diffForHumans() }}</small>
                                    </td>
                                    <td>
                                        <a href="{{ route('users.show', $user) }}" 
                                           class="btn btn-sm btn-outline-info" data-bs-toggle="tooltip" title="عرض">
                                           <i class="ti ti-eye"></i>
                                        </a>
            ...
            @if($users->hasPages())
                <div class="card-footer">
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="text-muted">عرض {{ $users->firstItem() }} إلى {{ $users->lastItem() }} من أصل {{ $users->total() }} نتيجة</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header border-0">
                <h5 class="modal-title text-danger"><i class="ti ti-alert-triangle me-2"></i>تأكيد الحذف</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body text-center">
                <i class="ti ti-trash display-1 text-danger mb-3"></i>
                <h6>هل أنت متأكد من حذف المستخدم؟</h6>
                <p class="text-muted mb-0">سيتم حذف المستخدم <strong id="deleteUserName"></strong> نهائياً ولا يمكن التراجع.</p>
            </div>
            <div class="modal-footer border-0 justify-content-center">
                <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal"><i class="ti ti-x me-1"></i>إلغاء</button>
                <form id="deleteForm" method="POST">
                    @csrf @method('DELETE')
                    <button type="submit" class="btn btn-danger"><i class="ti ti-trash me-1"></i>حذف نهائي</button>
                </form>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="toggleStatusModal" tabindex="-1" aria-labelledby="toggleStatusModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header border-0">
                <h5 class="modal-title"><i class="ti ti-user-check me-2"></i>تغيير حالة المستخدم</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body text-center">
            ...
            <div class="modal-footer border-0 justify-content-center">
                ...
                <form id="toggleStatusForm" method="POST">
                    @csrf @method('PATCH')
                    <button type="submit" class="btn" id="toggleStatusBtn"><i class="ti ti-check me-1"></i>تأكيد</button>
                </form>
            </div>
        </div>
    </div>
